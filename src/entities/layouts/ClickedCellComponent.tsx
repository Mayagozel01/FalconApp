...
const ClickedCellComponent = ({
  ...
  const firstCell = useMemo(() => {
    ...
    if (
      !firstLayout?.content?.structure?.rows ||
      firstLayout.content.structure.rows.length === 0
    )
      return null;
  ...
  const groupedHeaders = useMemo(() => {
    const groups = new Map<
      string,
      { parent: Header | null; children: Header[] }
    >();
  ...
  const rootCells = cells.filter(
    (cell) => !headers.some((header) => header.id === cell.headerId)
  );
            ...
            >
              ...
              {firstCell.state?.veracity !== undefined && (
                <FontAwesomeIcon
                  ...
                  style={{
                    color:
                      firstCell.state?.veracity === true
                        ? "green"
                        : firstCell.state?.veracity === false
                        ? "red"
                        : "yellow",
                    fontSize: "15px",
                  }}
        ...
        <tbody>
          {rootCells.length > 0 && (
            <>
              <tr>
                <td
                  colSpan={2}
                  style={{ padding: "8px", borderBottom: "2px solid black" }}
                >
          ...
          {groupedHeaders.map((group) => (
            <React.Fragment key={group.parent?.id || "root"}>
                    ...
                    <div
                      ...
                      onClick={() =>
                        ...
                        setExpandedGroups((prev) => ({
                          ...prev,
                          [group.parent.id]: !prev[group.parent.id],
                        }))
                      }
                    >
              ...
              {cells
                .filter((cell) => cell.headerId === group.parent?.id)
                .map((cell) => (
                  <tr key={cell.headerId}>
                    <td
                      style={{
                    ...
                    >
                      {cell.value || (
                        <span style={{ color: "#999" }}>Пусто</span>
                      )}
                    </td>
                  </tr>
                ))}
